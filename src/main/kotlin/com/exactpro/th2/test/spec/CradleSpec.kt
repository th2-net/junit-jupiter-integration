...
public class CradleSpec private constructor(
    ...
    public fun withPageSize(size: Int): CradleSpec = apply {
        require(size > 1) { "page size must be greater than 1" }
        settings.resultPageSize = size
    }

    public fun withRefreshBookInterval(intervalMillis: Long): CradleSpec = apply {
        settings.bookRefreshIntervalMillis = intervalMillis
    }

    public fun withQueryTimeout(timeout: Long): CradleSpec = apply {
        require(timeout > 0) { "timeout must be positive" }
        settings.timeout = timeout
    }

    public fun disableAutoPages(): CradleSpec = apply {
        autoPages = false
    }

    public fun withAutoPageInterval(interval: Duration): CradleSpec = apply {
    ...
    public fun withPageDuration(duration: Duration): CradleSpec = apply {
    ...
    public fun reuseKeyspace(): CradleSpec = apply {
        reuseKeyspace = true
    }
